...
def test_agent_loop_injects_real_stock_technical_adapter(tmp_path) -> None:
    ...
    assert loop.stock_selection_orchestrator.technical_data is not None
    assert (
        loop.stock_selection_orchestrator.technical_data.__class__.__name__
        == "MCPTechnicalDataAdapter"
    )
